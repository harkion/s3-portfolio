export default function ContactPage() {
  return (
    <section className="max-w-3xl">
      <h2 className="text-5xl font-semibold">Lets create something amazing together</h2>
      <p className="text-zinc-300 mt-4">
        I am open to new projects, creative ideas, or opportunities to be part of your vision.
      </p>

      <div className="mt-8 space-y-4">
        <a className="card block" href="mailto:gencfahrican@outlook.com">gencfahrican@outlook.com</a>
        <a className="card block" href="https://linkedin.com/in/genc-fahri-can" target="_blank">LinkedIn</a>
        <a className="card block" href="https://github.com/harkion" target="_blank">GitHub</a>
        <a className="card block" href="https://instagram.com/gencfahrican" target="_blank">Instagram</a>
      </div>
    </section>
  );
}
